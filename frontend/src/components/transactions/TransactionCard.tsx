...
const TransactionCard = ({ transaction }: { transaction: Transaction }) => {
  ...
  const [destAccount, setDestAccount] = useState<Account | undefined>(
    undefined,
  );
  ...
  useEffect(() => {
    const getAccount = async (
      accountId: number,
      updateState: typeof setSrcAccount | typeof setDestAccount,
    ) => {
  ...
  return (
    <>
      <li className="transactionCardContainer">
        <Link to={destinationUrl} state={[srcAccount?.name, destAccount?.name]}>
          <p> From </p>
          <p>{srcAccount?.name}</p>
